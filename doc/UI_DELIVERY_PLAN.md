...
| 1.2b | Hex grid overlay | 12 | **done both platforms** — real H3 cells with boundary geometry, tiled across the viewport as frozenApp did: blue outline everywhere, translucent green fill only where measurements exist. Observed on iOS 17.5 sim and `Medium_Phone_API_36`. Count label and parent→child drill-down match too, so the overlay is now at parity with frozenApp. |
