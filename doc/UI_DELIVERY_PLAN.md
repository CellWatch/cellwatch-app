...
| 1.2a-followup | Map points wired to the database — the shells built a `MapHomeViewModel` and never called `onMeasurementsLoaded`, so the map had no points at all, upstream of the icon defect | 2 | **done both platforms** — pin observed on the iOS simulator and on `Medium_Phone_API_36`, both at the simulated coordinates. Confirming the Android one required fixing location capture first (see below). |
| 1.2b | Hex grid overlay | 12 | **done both platforms** — real H3 cells with boundary geometry, tiled across the viewport as frozenApp did: blue outline everywhere, translucent green fill only where measurements exist. Observed on iOS 17.5 sim and `Medium_Phone_API_36`. Not yet matched to frozenApp: the per-cell measurement count label, and parent→child drill-down on tap. |
...
| 2.4 | Export | 11 | **done both platforms** — two formats: the FCC bulk-submission document ported unchanged from frozenApp, and an extended export carrying everything the FCC form cannot. Android writes through the Storage Access Framework, iOS through the share sheet. Verified on `Medium_Phone_API_36` (19.6 KB file written and read back) and iOS 17.5 sim. |
